...
function activityColor(activity: Resident['currentActivity']): string {
  switch (activity) {
    case 'walking': return '#4CAF50'
    case 'resting': return '#2196F3'
    case 'alert': return '#F44336'
    case 'offline': return 'rgba(49,55,43,0.25)'
  }
}

function activityLabel(activity: Resident['currentActivity']): string {
  switch (activity) {
...
function BatteryDot({ level, connected }: { level: number; connected: boolean }) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
      ...
      <Text style={{ fontFamily: 'NunitoSans_700Bold', fontSize: 10, color, opacity: 0.9 }}>
        {level}%
      </Text>
    </View>
  )
...
function ResidentCard({
  ...
  const hasAlert = resident.currentActivity === 'alert' ||
    resident.fallEvents.some(f => !f.resolved)
  ...
  return (
    ...
    >
      <View style={cardStyles.topRow}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[cardStyles.name, hasAlert && cardStyles.nameAlert]}>
            {resident.name}
          </Text>
          <Text style={[cardStyles.meta, hasAlert && cardStyles.metaAlert]}>
            {resident.room} · Age {resident.age}
          </Text>
        </View>
        <View style={[cardStyles.activityBadge, { backgroundColor: activityColor(resident.currentActivity) + '18' }]}>
          <View style={[cardStyles.activityDot, { backgroundColor: activityColor(resident.currentActivity) }]} />
          <Text style={[cardStyles.activityText, { color: activityColor(resident.currentActivity) }]}>
            {activityLabel(resident.currentActivity)}
          </Text>
        </View>
      </View>
      ...
      <View style={cardStyles.bottomRow}>
        ...
        <Text style={[cardStyles.lastSeen, hasAlert && cardStyles.lastSeenAlert]}>
          Seen {timeAgo(resident.lastSeen)}
        </Text>
        <View style={{ flex: 1 }} />
        {resident.fallEvents.length > 0 && (
          <View style={[cardStyles.fallBadge, unresolvedFalls > 0 && cardStyles.fallBadgeUnresolved]}>
            <Ionicons
              ...
              size={10}
              color={unresolvedFalls > 0 ? '#F44336' : colors.ink}
            />
...
const cardStyles = StyleSheet.create({
  ...
  activityText: { fontFamily: 'NunitoSans_800ExtraBold', fontSize: 10, letterSpacing: 0.3 },
        ...
        >
          <View style={detailStyles.heroCard}>
            ...
            <Text style={detailStyles.heroMeta}>
              {resident.room} · Age {resident.age} · {resident.facility}
            </Text>
            <View style={[detailStyles.activityPill, { backgroundColor: activityColor(resident.currentActivity) + '20' }]}>
              <View style={[detailStyles.activityDot, { backgroundColor: activityColor(resident.currentActivity) }]} />
              <Text style={[detailStyles.activityLabel, { color: activityColor(resident.currentActivity) }]}>
                {activityLabel(resident.currentActivity)}
              </Text>
            </View>
          </View>
            ...
            <View style={detailStyles.deviceRow}>
              <View style={[detailStyles.deviceIcon, { backgroundColor: resident.deviceConnected ? 'rgba(76,175,80,0.12)' : 'rgba(49,55,43,0.07)' }]}>
                <Ionicons
                  name="bluetooth"
                  size={18}
                  color={resident.deviceConnected ? '#4CAF50' : 'rgba(49,55,43,0.3)'}
                />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <View key={event.id} style={detailStyles.fallRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={detailStyles.fallTime}>
                      {event.timestamp.toLocaleDateString('en-MY', {
              ...
              <View key={contact.id} style={[detailStyles.contactRow, contact.isPrimary && detailStyles.contactRowPrimary]}>
                <View style={[detailStyles.contactIcon, contact.isPrimary && detailStyles.contactIconPrimary]}>
                  <Ionicons
                    ...
                    size={16}
                    color={contact.isPrimary ? colors.bg : colors.ink}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[detailStyles.contactName, contact.isPrimary && detailStyles.contactNameLight]}>
                    {contact.name}
                  </Text>
                  ...
                  <Text style={[detailStyles.contactPhone, contact.isPrimary && detailStyles.contactPhoneLight]}>
                    {contact.phone}
                  </Text>
                </View>
              </View>
...
const detailStyles = StyleSheet.create({
  ...
  activityLabel: { fontFamily: 'NunitoSans_800ExtraBold', fontSize: 11 },
  alertBanner: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
        ...
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryChip}>
            <Text style={styles.summaryNum}>
              {MOCK_RESIDENTS.filter(r => r.deviceConnected).length}
            </Text>
            <Text style={styles.summaryLabel}>Online</Text>
          </View>
          <View style={[styles.summaryChip, totalAlerts > 0 && styles.summaryChipAlert]}>
            <Text style={[styles.summaryNum, totalAlerts > 0 && styles.summaryNumAlert]}>
              {totalAlerts}
            </Text>
            <Text style={[styles.summaryLabel, totalAlerts > 0 && styles.summaryLabelAlert]}>
              Open Alerts
            </Text>
          </View>
        </View>
